...
$folder_karang_taruna = "uploads";
if (!file_exists($folder_karang_taruna)) {
    mkdir($folder_karang_taruna, 0777, true);
}

// Default lkk_id (sesuai dengan tabel karang_taruna)
$lkk_id = 6; // Pastikan id ini ada di tabel karang_taruna
...
if (isset($_POST["tambah"])) {
    ...
    $sql = "INSERT INTO karang_taruna (lkk_id, nama, alamat, jabatan, foto, no_hp) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isssss", $lkk_id, $nama, $alamat, $jabatan, $foto, $no_hp);
    $stmt->execute();
    $stmt->close();

    header("Location: adminkartar.php");
}

// Hapus Data
if (isset($_GET["hapus"])) {
    $id = $_GET["hapus"];
    $sql = "DELETE FROM karang_taruna WHERE id = ?";
    $stmt = $conn->prepare($sql);
...
if (isset($_GET["edit"])) {
    $id = $_GET["edit"];
    $sql = "SELECT * FROM karang_taruna WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
...
if (isset($_POST["update"])) {
    ...
    if ($_FILES["foto"]["name"]) {
        ...
        $sql = "UPDATE karang_taruna SET nama = ?, alamat = ?, jabatan = ?, foto = ?, no_hp = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $nama, $alamat, $jabatan, $foto, $no_hp, $id);
    } else {
        $sql = "UPDATE karang_taruna SET nama = ?, alamat = ?, jabatan = ?, no_hp = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
...
<div class="container mt-5">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h5><?= isset($editData) ? "Edit Karang Taruna" : "Tambah Karang Taruna" ?></h5>
            <form method="post" enctype="multipart/form-data">
                ...
                <?php if (isset($editData)): ?>
                    <a href="adminkartar.php" class="btn btn-secondary">Batal</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
...
<table class="table table-bordered">
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM karang_taruna WHERE lkk_id = $lkk_id";
            ...
            ?>
                <tr>
                    ...
                    <td><?= $row['no_hp'] ?></td>
                    <td>
                        <a href="adminkartar.php?edit=<?= $row['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
                        <a href="adminkartar.php?hapus=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data ini?')">Hapus</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
