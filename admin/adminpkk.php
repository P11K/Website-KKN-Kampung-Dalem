...
$folder_pkk = "uploads";
if (!file_exists($folder_pkk)) {
    mkdir($folder_pkk, 0777, true);
}

// Default lkk_id (sesuai dengan tabel lkk)
$lkk_id = 4; // Pastikan id ini ada di tabel lkk
...
if (isset($_POST["tambah"])) {
    ...
    $sql = "INSERT INTO pkk (lkk_id, nama, alamat, jabatan, foto, no_hp) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isssss", $lkk_id, $nama, $alamat, $jabatan, $foto, $no_hp);
    $stmt->execute();
    $stmt->close();

    header("Location: adminpkk.php");
}
...
if (isset($_POST["update"])) {
    ...
    if ($_FILES["foto"]["name"]) {
        ...
        $sql = "UPDATE pkk SET nama = ?, alamat = ?, jabatan = ?, foto = ?, no_hp = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $nama, $alamat, $jabatan, $foto, $no_hp, $id);
    } else {
        $sql = "UPDATE pkk SET nama = ?, alamat = ?, jabatan = ?, no_hp = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Admin pkk</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
</head>
<body>

<div class="container mt-5">
    <h2 class="text-center">Data pkk</h2>

    <!-- Form Tambah & Edit -->
    <div class="card mb-4">
        <div class="card-body">
            <h5><?= isset($editData) ? "Edit pkk" : "Tambah pkk" ?></h5>
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <?php if (isset($editData) && $editData['foto']): ?>
                        <img src="pkk/<?= $editData['foto'] ?>" width="50">
                    <?php endif; ?>
                </div>
                <div class="mb-3">
                    <label>No HP</label>
                ...
                <?php if (isset($editData)): ?>
                    <a href="adminpkk.php" class="btn btn-secondary">Batal</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM pkk WHERE lkk_id = $lkk_id";
            ...
            ?>
                <tr>
                    ...
                    <td><?= $row['no_hp'] ?></td>
                    <td>
                        <a href="adminpkk.php?edit=<?= $row['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
                        <a href="adminpkk.php?hapus=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data ini?')">Hapus</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
