...
$folder_rw = "rw";
if (!file_exists($folder_rw)) {
    mkdir($folder_rw, 0777, true);
}

// Default lkk_id (sesuai dengan tabel lkk)
$lkk_id = 3; // Pastikan id ini ada di tabel lkk
...
if (isset($_POST["tambah"])) {
    ...
    $sql = "INSERT INTO rw (lkk_id, nama, alamat, jabatan, foto, no_hp) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isssss", $lkk_id, $nama, $alamat, $jabatan, $foto, $no_hp);
    $stmt->execute();
    $stmt->close();

    header("Location: adminrw.php");
}
...
if (isset($_POST["update"])) {
    ...
    if ($_FILES["foto"]["name"]) {
        ...
        $sql = "UPDATE rw SET nama = ?, alamat = ?, jabatan = ?, foto = ?, no_hp = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $nama, $alamat, $jabatan, $foto, $no_hp, $id);
    } else {
        $sql = "UPDATE rw SET nama = ?, alamat = ?, jabatan = ?, no_hp = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
...
<div class="container mt-5">
    <h2 class="text-center">Data RW</h2>

    <!-- Form Tambah & Edit -->
    <div class="card mb-4">
        <div class="card-body">
            <h5><?= isset($editData) ? "Edit RW" : "Tambah RW" ?></h5>
            <form method="post" enctype="multipart/form-data">
                ...
                <?php if (isset($editData)): ?>
                    <a href="adminrw.php" class="btn btn-secondary">Batal</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM rw WHERE lkk_id = $lkk_id";
            ...
            ?>
                <tr>
                    ...
                    <td><?= $row['no_hp'] ?></td>
                    <td>
                        <a href="adminrw.php?edit=<?= $row['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
                        <a href="adminrw.php?hapus=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data ini?')">Hapus</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
